...
async function getGroups(request, response, next) {
  ...
  const {sort_by, sort_order} = getSort(request.query, {
    ...
    collection_name: 'zendesk_groups',
  });
  
  // Call the Zendesk API using the search endpoint
  return axios.get(`${ZENDESK_URL_PREFIX}/api/v2/groups`, {
    headers: {
  ...
  .then( async (resp) => {
    ...
    let records = resp.data.groups;
    // Serialize the result using the Forest Admin format
    const recordsSerializer = new RecordSerializer({ name: 'zendesk_groups' });
...
async function getGroup(request, response, next) {
  return axios.get(`${ZENDESK_URL_PREFIX}/api/v2/groups/${request.params.groupId}`, {
    headers: {
      'Authorization': `Basic ${getToken()}` 
    },
  })
  .then( async (resp) => {
    let record = resp.data.group;
    // Serialize the result using the Forest Admin format
    const recordSerializer = new RecordSerializer({ name: 'zendesk_groups' });
...
function getZendeskGroupById(groupId) {
  if(!groupId) return null;
  return axios.get(`${ZENDESK_URL_PREFIX}/api/v2/groups/${groupId}`, {
    headers: {
      'Authorization': `Basic ${getToken()}` 
    },
  })
